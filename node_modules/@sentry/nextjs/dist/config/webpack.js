...
function constructWebpackConfigFunction(userNextConfig, userSentryWebpackPluginOptions) {
    ...
    var newWebpackFunction = function (config, options) {
        ...
        if (config.target === 'node') {
            utils_2.storeServerConfigFileLocation(config);
        }
        ...
        new SentryWebpackPlugin(tslib_1.__assign(tslib_1.__assign({ dryRun: options.dev, release: node_1.getSentryRelease(options.buildId) }, defaultSentryWebpackPluginOptions), userSentryWebpackPluginOptions)));
        return newConfig;
    };
            ...
            switch (_a.label) {
                ...
                case 2:
                    ...
                    if (isServer) {
                        ...
                        newEntryProperty[utils_2.SERVER_SDK_INIT_PATH.slice(0, -3)] = utils_2.SENTRY_SERVER_CONFIG_FILE;
                    }
                    // On the client, it's sufficient to inject it into the `main` JS code, which is included in every browser page.
                    else {
                        addFileToExistingEntryPoint(newEntryProperty, 'main', utils_2.SENTRY_CLIENT_CONFIG_FILE);
                        ...
                        if (Array.isArray(mainjsValue) && mainjsValue.length > 0) {
                            ...
                            newEntryProperty.main = Array.isArray(mainValue)
                                ? tslib_1.__spread(mainjsValue, mainValue) : tslib_1.__assign(tslib_1.__assign({}, mainValue), { import: tslib_1.__spread(mainjsValue, mainValue.import) });
                            // nuke the entries
...
function addFileToExistingEntryPoint(entryProperty, entryPointName, filepath) {
    ...
    if (typeof injectedInto === 'string') {
        ...
        injectedInto = tslib_1.__spread(injectedInto, [filepath]);
    }
    else {
        ...
        if (typeof injectedInto.import === 'string') {
            ...
            importVal = tslib_1.__spread(injectedInto.import, [filepath]);
        }
